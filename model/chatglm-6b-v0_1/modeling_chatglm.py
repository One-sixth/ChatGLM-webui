...
class ChatGLMModel(ChatGLMPreTrainedModel):
    ...
    def forward(
            ...
            out_prefix_ids: Optional[torch.LongTensor] = None,
...
class ChatGLMForConditionalGeneration(ChatGLMPreTrainedModel):
    ...
    def forward(
            ...
            out_prefix_ids: Optional[torch.Tensor] = None,
    ...
    def chat(self, tokenizer, query: str, history: List[Tuple[str, str]] = None, max_length: int = 2048, num_beams=1,
             do_sample=True, top_p=0.7, temperature=0.95, logits_processor=None, out_prefix='', **kwargs):
        if history is None:
            history = []
        ...
        # new add
        out_prefix_ids = tokenizer([out_prefix], return_tensors="pt", padding=True)
        out_prefix_ids = out_prefix_ids.to(self.device)
        out_prefix_ids = out_prefix_ids['input_ids']
        if out_prefix_ids[0, 0].item() == 20005:
            out_prefix_ids = out_prefix_ids[:, 1:]
        out_prefix_ids = out_prefix_ids[:, :-2]
        #
        # outputs = self.generate(**input_ids, **gen_kwargs)
        outputs = self.generate(input_ids=input_ids['input_ids'], out_prefix_ids=out_prefix_ids, **gen_kwargs)
    ...
    def stream_chat(self, tokenizer, query: str, history: List[Tuple[str, str]] = None, max_length: int = 2048,
        ...
        for outputs in self.stream_generate(input_ids=input_ids['input_ids'], out_prefix_ids=out_prefix_ids, **gen_kwargs):
            outputs = outputs.tolist()[0][len(input_ids["input_ids"][0]):]
            response = tokenizer.decode(outputs)
            # 阻止回车符被吃掉
            # response = response.strip()
    ...
    def stream_generate(
        ...
        # process prefix
        input_ids = torch.cat([input_ids, out_prefix_ids], -1)
